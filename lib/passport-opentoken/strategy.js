...
var OpenTokenAPI = require('@jantaylor/opentoken').OpenTokenAPI;
 ...
 * set.
 *
 * @param {Object} options Options to change Strategy
 * @param {Function} verify Callback to verify Strategy
 * 
 * Options:
...
Strategy.prototype.authenticate = function (req, options) {
  ...
  var token = getParam(req, tokenName);

  if (!token) {
    return self.fail(util.format("OpenToken %s missing from request", tokenName));
  }
